        ...
        with st.spinner("Menganalisis data perilaku, struktur linguistik, dan mengecek API eksternal..."):
            ...
            if enable_l3:
                try:
                    ...
                    resp = requests.post(f"{backend_proxy_url}/api/detect", json={"text": sample_text, "provider": prov_id}, timeout=4)
                    if resp.status_code == 200:
                        data = resp.json()
                        l3_score = data.get("human_score", 90) if prov_id == "watsonx" else round((1 - data.get("probability", 0.1)) * 100)
                        l3_message = f"Terhubung ({l3_provider})"
                    else:
                        l3_message = f"API Proxy Error ({resp.status_code})"
                except Exception as e:
                    l3_message = f"Koneksi Proxy Offline ({backend_proxy_url})"
                    l3_score = 85  # Simulation fallback for demo UI
            ...
            with col2:
                verdict = "HUMAN_VERIFIED" if ensemble_score >= 75 else "MIXED_CONTENT" if ensemble_score >= 50 else "AI_DETECTED"
...
with tab4:
    ...
    | :--- | :--- | :--- |
    | **IBM watsonx.ai (Granite Model)** | Cloud Run Proxy / REST API | 🟢 Connected (us-south) |
